  ...
  try {
    ...
    for (let i = 0; i < jsonData.length; i++) {
      ...
      try {
        ...
        if (!fila.nombre || !fila.apellidos) {
          resultados.errores.push(`Fila ${i + 1}: Nombre y apellidos son requeridos`);
          continue;
        }
        ...
        if (fila.email) {
          const emailExistente = db.get('empleados')
            .find({ email: fila.email.toLowerCase(), activo: true })
            .value();
          
          if (emailExistente) {
            resultados.errores.push(`Fila ${i + 1}: Email ${fila.email} ya existe`);
            continue;
          }
        }
        
        // Procesar roles (si vienen como string separado por comas)
        let roles = [];
        if (fila.roles) {
          roles = fila.roles.split(',').map(r => r.trim());
          if (!validateRoles(roles)) {
            resultados.errores.push(`Fila ${i + 1}: Roles inválidos`);
            continue;
          }
        }
        ...
        const nuevoEmpleado = {
          ...
          email: fila.email ? fila.email.toLowerCase() : null,
          telefono: fila.telefono || null,
          puesto: fila.puesto || null,
          departamento: fila.departamento || null,
          salario: fila.salario ? parseFloat(fila.salario) : null,
          fechaIngreso: fila.fechaIngreso || new Date().toISOString().split('T')[0],
          numeroEmpleado: fila.numeroEmpleado || null,
          roles: roles,
          tieneUsuario: false,
          activo: true,
          fechaRegistro: new Date().toISOString(),
        ...
        db.get('empleados').push(nuevoEmpleado).write();
        resultados.creados.push(nuevoEmpleado);
    ...
    res.json(
      createResponse(
        true,
        resultados,
        `Importación completada: ${resultados.importados} empleados importados`
      )
    );
